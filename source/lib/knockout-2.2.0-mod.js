...
ko.exportSymbol('jsonExpressionRewriting.insertPropertyAccessorsIntoJson', ko.expressionRewriting.preProcessBindings);(function() {
    ...
    var startCommentRegex = commentNodesHaveTextProperty ? /^<!--\s*uncrd(?:\s+(.+\s*\:[\s\S]*))?\s*-->$/ : /^\s*uncrd(?:\s+(.+\s*\:[\s\S]*))?\s*$/;
    var endCommentRegex =   commentNodesHaveTextProperty ? /^<!--\s*\/uncrd\s*-->$/ : /^\s*\/uncrd\s*$/;
...
(function() {
    var defaultBindingAttributeName = "data-uncrd";
...
ko.templateRewriting = (function () {
    var memoizeDataBindingAttributeSyntaxRegex = /(<[a-z]+\d*(\s+(?!data-uncrd=)[a-z0-9\-]+(=(\"[^\"]*\"|\'[^\']*\'))?)*\s+)data-uncrd=(["'])([\s\S]*?)\5/gi;
